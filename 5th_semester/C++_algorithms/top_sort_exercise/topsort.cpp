...
std::pair<bool, std::vector<Vertex>> topsorting(const Graph& G, std::vector<bool>& visited, std::vector<Vertex> & res, const Vertex& v, std::vector<Vertex> hist, std::vector<bool> frodo){
    hist.push_back(v);
    frodo[size_t(v)] = true;
    std::pair<bool, std::vector<Vertex>> tmp;
    ...
    for(Vertex u : G[v]){
        if(visited[size_t(u)]) {
            if (frodo[size_t(u)]) {
                ...
                return std::make_pair(false,hist);
            }
            else continue;
        }
        tmp = topsorting(G, visited, res, u, hist, frodo);
        if(!tmp.first) return tmp;
    }
    res.push_back(v);
    return std::make_pair(true, hist);
}
...
std::pair<bool, std::vector<Vertex>> topsort(const Graph& G) {
    ...
    std::vector<Vertex> hist;
    std::vector<bool> visited(G.vertices(), false);
    std::pair<bool, std::vector<Vertex>> tmp;
    std::vector<bool> frodo(G.vertices(), false);
    for(Vertex v : G){
        if(visited[size_t(v)]) continue;
        tmp = topsorting(G, visited, res, v, hist, frodo);
        if(!tmp.first){
            return tmp;
        }
    }
